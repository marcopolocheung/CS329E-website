...
def add_admin():
    ...
    if admin != None:
        ...
        bcryptPassword = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
        admin = Admin(name=name, password=bcryptPassword.decode('utf-8'))
        session.add(admin)
        session.commit()
...
def add_user():
    ...
    bcryptPassword = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
    newuser = User(name=name, password=bcryptPassword.decode('utf-8'))
...
def login():
    ...
    if username != '':
        ...
        if users.count() == 0:
            ...
            if not bcrypt.checkpw(password.encode('utf-8'), storedPassword):
                return render_template('not-found.html', user=username)    
    ...
    if 'credentials' in flask.session:
        ...
        dbsession = DBSession()
        existingUser = dbsession.query(User).filter_by(name=username).first()
        if existingUser is None:

            oauth_user = User(name=username, password='')
            dbsession.add(oauth_user)
            dbsession.commit()
