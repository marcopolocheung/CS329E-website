...
class ETL():
    ...
    def _load_data(self):
        ...
        for city in cities:
            ...
            #######4DONE####### 1. Assignment 4 TODO: Check if the city has data available by checking for return code 404
            if r.status_code == 404:
                app.logger.info(f"No weather data for {city_name}. STATUSCODE 404!!!")
                continue
...
def city_status_graph():
    ...
    params = usercity.weather_params.split(',')
    
    for param in params:

        key = year + "-" + month + "-" + param
        weather_param = dbsession.query(WeatherParameter).filter_by(
            cityId=city.id, 
            year_month_param=key
        ).first()
        
        if weather_param != None:
            op[key] = weather_param.values
